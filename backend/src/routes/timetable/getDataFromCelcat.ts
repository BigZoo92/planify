...
export async function scrapePage(urlToScrape: string) {
  try {
    const { dataPage } = await scraper(urlToScrape);
    if (dataPage) {
      const hash = crypto.createHash('sha256').update(dataPage).digest('hex');
      return { dataPage, hash };
    } else {
...
export function parseEvents(dataPage: string) {
  ...
  const events = eventsPerDaysXml
    .flatMap((day) => {
      ...
      const date = dateId
        ? dates.find((e) => e.weekStart == weekStart)?.dates[parseInt(dateId)]
        : null;
      return [
        {
          data: {
