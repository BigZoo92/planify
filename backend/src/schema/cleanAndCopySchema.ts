...
const backendDestPath = path.join(__dirname, '../../../backend/src/schema/index.ts');
const frontendDestPath = path.join(__dirname, '../../../frontend/src/schema/index.ts');
...
async function cleanSchema() {
  try {
    ...
    const endIdx = lines.findIndex((line, index) => line.includes('// ENUMS') && index > startIdx);

    // Supprimer la section non désirée et nettoyer les références 'Prisma'
    const newData = [...lines.slice(0, startIdx), ...lines.slice(endIdx + 1)].join('\n');
